...
"""
[File Description]
This script is STEP 2 of the data generation pipeline.
It is an Isaac Lab application that takes ONE intermediate JSON file, 
launches the physics simulation, spawns the blocks, captures multiple camera views,
and saves the final images alongside a unified `*data.json` into the dataset folder.
Original name: batch_gen.py
Usage: python step2_render_images_single_v2.py --enable_cameras --type <category> --json_file <filename.json>
"""
...
parser.add_argument("--type", type=str, choices=["small", "small_size4", "large"], default="large", help="Category: small, small_size4 or large")
...
# 指定 JSON 文件的路径
DATASET_DIR = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../../assets/dataset_v2"))
JSON_FOLDER = os.path.join(DATASET_DIR, "intermediate_json", args_cli.type)
...
CUBE_SIZE = 0.05
GAP_SIZE = 0.001
CELL_SIZE = CUBE_SIZE + GAP_SIZE  # 0.051m

GRID_SIZE = 8  # 网格行列数 (8x8 坐标系统：A-H, 1-8)
GRID_ORIGIN = [-(GRID_SIZE / 2.0) * CELL_SIZE, -(GRID_SIZE / 2.0) * CELL_SIZE, TABLE_HEIGHT + GAP_SIZE]  # 网格原点(0,0)位置 - 左下角
...
class SnapshotSceneCfg(InteractiveSceneCfg):
    ...
    # 2. 光照 (使用穹顶光 DomeLight 提供 360 度均匀无死角的漫反射环境光)
    # 这样可以避免顶部死白亮光，并且让 Front 和 Side 也被均匀照亮
    dome_light = AssetBaseCfg(
        prim_path="/World/dome_light",
        spawn=sim_utils.DomeLightCfg(
            intensity=1500.0, # 均匀亮度，可以根据需要调节
            color=(1.0, 1.0, 1.0)
        )
    )

    # 3. 辅助侧光 (可以略微增加层次感，从斜角打光)
...
def add_blocks_from_coords(scene_cfg_cls, coords_list, origin):
    ...
    def grid_to_world_pos(grid_x, grid_y, grid_z):
        ...
        world_x = origin[0] + grid_x * CELL_SIZE + (CELL_SIZE / 2.0)
        world_y = origin[1] + grid_y * CELL_SIZE + (CELL_SIZE / 2.0)
        world_z = TABLE_HEIGHT + CUBE_SIZE/2 + grid_z * CUBE_SIZE
        return (world_x, world_y, world_z)
    
    # 贴图路径
    texture_path = os.path.abspath(os.path.join(DATASET_DIR, "bordered_blue.png"))
    
    for i, (gx, gy, gz) in enumerate(coords_list):
        world_pos = grid_to_world_pos(gx, gy, gz)
        
        # 使用真实带有独立视觉缩放的 USD 模型 (视觉小一圈，物理碰边大一圈)
        cube_cfg = RigidObjectCfg(
            prim_path=f"{{ENV_REGEX_NS}}/Block_{i}",
            spawn=sim_utils.UsdFileCfg(
                usd_path=os.path.abspath(os.path.join(DATASET_DIR, "bordered_blue_block.usda")),
                scale=(1.0, 1.0, 1.0),
            ),
...
def add_coordinate_grid_to_scene_config(scene_cfg_cls, origin, grid_size=GRID_SIZE, cell_size=CELL_SIZE, gap_width=GAP_SIZE):
    """
    ...
    for i in range(grid_size + 1):
        ...
        h_line_cfg = AssetBaseCfg(
            ...
            spawn=sim_utils.CuboidCfg(
                size=(grid_size * cell_size, line_thickness, line_height),  # 线宽
        ...
        v_line_cfg = AssetBaseCfg(
            ...
            spawn=sim_utils.CuboidCfg(
                size=(line_thickness, grid_size * cell_size, line_height),  # 线宽
                visual_material=material,
                collision_props=None,
            ),
            init_state=AssetBaseCfg.InitialStateCfg(
    ...
    origin_marker_cfg = AssetBaseCfg(
        ...
        init_state=AssetBaseCfg.InitialStateCfg(
            pos=(origin[0], origin[1], origin[2] + GAP_SIZE)  # 稍微抬高以突出显示
        )
    )
...
def main():
    ...
    # ==============================================================
    # 新增：强行将多个 Camera 切换为正交投影 (Orthographic)
    # ==============================================================
    from pxr import UsdGeom
    
    # 我们要修改的相机列表
    ortho_cameras = ["Camera", "CameraFront", "CameraSide"]
    
    # 遍历所有环境 
    for env_idx in range(args_cli.num_envs):
        for cam_name in ortho_cameras:
            cam_path = f"/World/envs/env_{env_idx}/{cam_name}" 
            cam_prim = UsdGeom.Camera(sim.stage.GetPrimAtPath(cam_path))
            
            if cam_prim.GetPrim().IsValid():
                # 1. 强行修改投影模式为正交 (对应你查到的 API 功能)
    ...
    # 计算世界坐标用于记录
    def grid_to_world_pos(grid_x, grid_y, grid_z):
        world_x = GRID_ORIGIN[0] + grid_x * CELL_SIZE + (CELL_SIZE / 2.0)
        world_y = GRID_ORIGIN[1] + grid_y * CELL_SIZE + (CELL_SIZE / 2.0)
        world_z = TABLE_HEIGHT + CUBE_SIZE/2 + grid_z * CUBE_SIZE
