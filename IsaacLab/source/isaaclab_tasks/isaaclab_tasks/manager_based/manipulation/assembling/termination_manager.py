...
def _resolve_total_blocks(task_spec: Any) -> int:
    if task_spec is None:
        return 0
    return max(0, int(getattr(task_spec, "total_blocks", 0)))
...
class IsaacTerminationManager:
    """Episode termination rules for the assembling task runtime."""
    ...
    def reset(self, task_spec: Any) -> None:
        total_blocks = _resolve_total_blocks(task_spec)
        self._placement_attempts = 0
        self._max_attempts = max(
            1,
            int(math.ceil(max(1, total_blocks) * float(self.config.max_attempts_factor))),
        )
