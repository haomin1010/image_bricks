...
HARDCODED_OPENAI_MODEL = "gpt-5.4"
HARDCODED_OPENAI_API_BASE = "http://35.220.164.252:3888/v1"
HARDCODED_OPENAI_API_KEY = "sk-L6li3GPIf7dGQk0j0YxUnC1qOLDhLzVSEfh9frOs9URBUYsL"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        type=str,
        default=HARDCODED_OPENAI_MODEL,
        help=f"Model name sent to the hardcoded endpoint. Default: {HARDCODED_OPENAI_MODEL}",
    )
    parser.add_argument(
        "--reasoning_effort",
        type=str,
        ...
        help="Unused by the hardcoded chat.completions path. Kept only for CLI compatibility.",
    )
    parser.add_argument(
        "--image_detail",
        type=str,
...
def _build_actual_user_content(
    ...
    content: List[dict] = []
    _ = image_detail
    content.append({"type": "text", "text": "Now generate the explanation for the following training example."})
    ...
    for item in user_message:
        ...
        if item_type == "text":
            ...
            content.append(
                {
                    "type": "image_url",
                    "image_url": {"url": _image_to_data_url(abs_path.as_posix())},
                }
            )
        else:
    ...
    if overlay_paths:
        ...
        for view_name, abs_path in overlay_paths.items():
            content.append({"type": "text", "text": f"Overlay view {view_name}:"})
            content.append(
                {
                    "type": "image_url",
                    "image_url": {"url": _image_to_data_url(abs_path)},
                }
            )
...
def _get_thread_openai_client():
    ...
    if client is None:
        ...
        print(f"apikey: {HARDCODED_OPENAI_API_KEY} base: {HARDCODED_OPENAI_API_BASE}")
        client = OPENAI_CLIENT_CLASS(api_key=HARDCODED_OPENAI_API_KEY, base_url=HARDCODED_OPENAI_API_BASE)
...
def _build_cot_record(
    ...
    record["meta"]["cot_source"] = "openai_chat_completions_api"
...
def _call_openai_reasoning(
    ...
    if finish_reason == "length":
        raise ValueError("Chat completion stopped early because finish_reason=length.")
    output_text = _chat_completion_text(choice.message.content)
    return output_text, response


def _is_max_output_tokens_incomplete(exc: Exception) -> bool:
    text = str(exc).lower()
    return ("incomplete_details" in text and "max_output_tokens" in text) or "finish_reason=length" in text
...
def main() -> None:
    ...
    print(f"[INFO] api_base={HARDCODED_OPENAI_API_BASE}")
